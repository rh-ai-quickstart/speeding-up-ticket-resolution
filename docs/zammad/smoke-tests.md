...
2. **Route:** `oc get route ssa-zammad -n "${NAMESPACE}" -o jsonpath='{.spec.host}{"\n"}'` — open **`https://<host>`** (login per upstream demo defaults or your replacements in [`helm/values-ticketing.yaml`](https://github.com/rh-ai-quickstart/it-self-service-agent/blob/1c37753dbffdf4a79b8f2fbca47bc539dbf459ce/helm/values-ticketing.yaml)).
