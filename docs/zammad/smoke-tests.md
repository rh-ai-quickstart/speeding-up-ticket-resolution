...
2. **Route:** `oc get route ssa-zammad -n "${NAMESPACE}" -o jsonpath='{.spec.host}{"\n"}'` — open **`https://<host>`** (login per upstream demo defaults or your replacements in [`helm/values-ticketing.yaml`](https://github.com/rh-ai-quickstart/it-self-service-agent/blob/5af29a5b8780d7de448135bf45a0fda247799736/helm/values-ticketing.yaml)).
